...
std::vector<at::Tensor> BallQuery(
  ...
  TORCH_CHECK(points.size(1) == 3, "points.size(1) must be 3 but got ", points.size(1));
  TORCH_CHECK(centroids.size(1) == 3, "centroids.size(1) must be 3 but got ", centroids.size(1));
  ...
  TORCH_CHECK(cudaGetLastError() == cudaSuccess, "CUDA kernel failed: ", cudaGetErrorString(cudaGetLastError()));
